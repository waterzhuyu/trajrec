...
class SpecifiedEncoder(nn.Module):
    def __init__(self, hid_dim, pro_output_dim, input_dim, dropout):
        ...
        input_dim = input_dim 
        self.input_cat = nn.Linear(64 * 2, hid_dim, bias = False)
        self.relu = nn.ReLU(inplace=True)
...
class MM_STGED(nn.Module):
    def __init__(self, encoder, decoder, shared_emb, num_emb, id_size, hid_dim, id_emb_dim, base_channel, x_id, y_id, top_k):
        ...
        self.encoder_out = nn.Sequential(
            nn.Linear(512+256+512, 512), #TODO: change this line!
            nn.ReLU(inplace=True),
            nn.Linear(512, 512)
        )
    ...
    def forward(self, user_tf_idf, spatial_A_trans, road_condition, src_road_index_seqs, SE, 
        ...
        src_specified_attention, src_specified_hidden = self.mygcn(src_specified_attention, tra_time_A, tra_loca_A)
