...
from utils.candidate_point import get_candidates, CandidatePoint
...
def rate2gps(rn_dict, eid, rate, parameters):
    ...
    for i in range(len(coords) - 1):
        current_distance = distance(coords[i], coords[i + 1])
        if current_distance == 0:
            print(f"Warning: Zero distance encountered between {coords[i]} and {coords[i + 1]}, skipping this pair.")
            continue  # Skip this pair of coordinates

        if i > 0:
            pre_dist += distance(coords[i - 1], coords[i])
        dist += current_distance
        if dist >= offset:
            coor_rate = (offset - pre_dist) / current_distance
            project_pt = cal_loc_along_line(coords[i], coords[i + 1], coor_rate)
            return project_pt  # Return the computed project_pt

    # 如果没有找到合适的点（这不太可能发生，除非所有距离都为零），返回最后一个坐标
    return coords[-1]
