...
from .parse_traj import ParseMMTraj_Raw_eid
...
class Dataset(torch.utils.data.Dataset):
    ...
    def __init__(self, trajs_dir, user_idx, raw2new_rid, mbr, norm_grid_poi_dict, norm_grid_rnfea_dict, weather_dict, parameters, debug=True):
    ...
    def get_data(self, trajs_dir, user_idx, raw2new_rid, norm_grid_poi_dict, norm_grid_rnfea_dict,
                 weather_dict, win_size, ds_type, keep_ratio, debug):
        parser = ParseMMTraj_Raw_eid()
        ...
        for file_name in tqdm(trg_paths):
            trajs, user_id = parser.parse(os.path.join(trajs_dir, file_name), user_idx, raw2new_rid)
            user_id_cnt = 0
